...
function artworkBestUrl(track) {
	...
	if (!art && track.publisher_metadata && track.publisher_metadata.artwork && track.publisher_metadata.artwork.url) {
...
async function load(by) {
	...
	btn.el.onclick = async () => {
		try {
			const progressive = result.media && result.media.transcodings && result.media.transcodings.find(t => t.format && t.format.protocol === 'progressive')
			if (!progressive) {
			...
			try {
				...
				if (result.user && result.user.username) writer.setFrame('TPE1', [result.user.username])
				if (result.publisher_metadata && result.publisher_metadata.album_title) {
				...
				if (coverBuf) {
					...
					if (dv[0] === 0x89 && dv[1] === 0x50 && dv[2] === 0x4E) mime = 'image/png'
			...
			try {
				...
				if (readable.pipeTo) {
					...
					const pump = () => reader.read().then(({done, value}) => done ? writer.close() : writer.write(value).then(pump))
					await pump()
				}
			} catch (e) {
